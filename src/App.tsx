...
import { 
  MOCK_PROVIDERS, MOCK_USERS, MOCK_COMMUNITY_POSTS, MOCK_NOTIFICATIONS 
} from './constants';
import { 
...
const LoginPage = ({ onLoginSuccess }: { onLoginSuccess: (user: any) => void }) => {
  // role: 'user' | 'provider' (Initial choice)
  // social: 'user' flow social login options
  // email: 'user' flow email login form
  // partner-auth: 'provider' flow email/business form
  ...
  const handleRoleSelect = (role: UserRole) => {
    ...
    if (role === 'provider') {
        // Partners skip social login and go straight to dedicated email form
        setIsSignUp(false); // Default to login view
        setStep('partner-auth');
    } else {
        // Users go to social login options
        setStep('social');
    }
  };

  const completeLogin = (user: any) => {
    // If the user object explicitly has a role (e.g. from admin mock), prefer it.
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    if (isSignUp) {
        ...
        // Signup strictly requires email
        if (!email.includes('@')) {
    ...
    if (isSignUp) {
      ...
      // Special Logic: Allow non-email login for Partners (Legacy/Admin IDs)
      if (!isEmail && selectedRole === 'provider') {
          // Simulate API call delay for mock login
          await new Promise(resolve => setTimeout(resolve, 500));
          
          // Special Check: Admin ID
          if (email === 'admin') {
              result = {
                  success: true,
                  user: {
              ...
              // Assume it's a legacy provider ID
              result = {
                  success: true,
                  user: {
                      uid: `provider_${email}`,
                      email: null,
                      displayName: email, // Use ID as display name initially
                      photoURL: null,
                      role: 'provider'
                  }
              };
          ...
          // Standard Email Login (Firebase)
          result = await loginWithEmail(email, password);
      }
    }

    if (result.success && result.user) {
      // Inject extra fields for partners if signing up
  ...
  // --- Render Step 2 (User): Social Login ---
  if (step === 'social') {
    return (
      <div className="min-h-screen bg-white flex flex-col animate-fade-in relative">
         <button 
            onClick={() => setStep('role')} 
            className="absolute top-4 left-4 p-2 text-gray-400 hover:text-gray-800"
         >
           <ArrowLeft size={24} />
         </button>
         ...
         <div className="flex-1 flex flex-col justify-center px-8">
            ...
            <div className="space-y-3 mb-6">
              <button 
                  ...
                  className="w-full bg-[#FEE500] text-[#3c1e1e] py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 hover:opacity-90 transition-opacity shadow-sm"
              >
                <MessageCircle fill="currentColor" size={20} /> 카카오로 3초만에 시작하기
              </button>
              <button 
                  onClick={() => handleSocialLogin('google')}
                  className="w-full bg-white border border-gray-200 text-gray-700 py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-gray-50 transition-colors shadow-sm"
              >
            ...
            <div className="text-center">
               <button 
                 ...
                 className="text-gray-400 text-xs underline underline-offset-4 hover:text-gray-600"
               >
                 이메일 로그인
               </button>
            </div>
           ...
           <form onSubmit={handleEmailAuth} className="space-y-3">
              {isSignUp && (
                 <input 
                   type="text" 
                   placeholder="이름 (닉네임)"
                   className="w-full border border-gray-300 rounded-xl px-4 py-3 outline-none focus:border-[#2AC1BC]"
                   value={name}
              ...
              <button 
                 type="submit" 
                 disabled={isLoading}
                 className="w-full text-white font-bold py-3.5 rounded-xl transition-colors shadow-md bg-[#2AC1BC] hover:bg-[#209692]"
              >
  ...
  // --- Render Step 3 (Partner): Dedicated Email/Business Form ---
  if (step === 'partner-auth') {
    return (
      <div className="min-h-screen bg-white flex flex-col px-8 animate-fade-in">
         <div className="h-14 flex items-center -mx-2">
           <button onClick={() => setStep('role')} className="p-2 text-gray-400 hover:text-gray-800">
             <ArrowLeft size={24} />
           </button>
         </div>
         ...
         <div className="flex-1 flex flex-col justify-center pb-20">
           <div className="mb-10">
             <h1 className="text-3xl font-bold mb-2 tracking-tight text-gray-900">
               {isSignUp ? '파트너스 가입 신청' : '파트너스 로그인'}
             </h1>
             <p className="text-gray-500 text-sm">
               {isSignUp ? '사업자 정보를 입력하여 가입을 신청해주세요.' : '성공적인 비즈니스의 시작'}
             </p>
           </div>
           ...
           <form onSubmit={handleEmailAuth} className="space-y-4">
              {isSignUp && (
                <div className="space-y-3 animate-fade-in-up">
                    <input 
                        type="text" 
                        placeholder="상호명 (예: 맘플산후조리)"
                        className="w-full border border-gray-300 rounded-xl px-4 py-3 outline-none focus:border-gray-900 bg-gray-50"
                        value={businessName}
              ...
              <input 
                type="text" // Allows simple IDs for Partners
                placeholder="이메일 (아이디)" 
              ...
              <button 
                 type="submit" 
                 disabled={isLoading}
                 className="w-full text-white font-bold py-4 rounded-xl transition-colors shadow-md bg-gray-900 hover:bg-black mt-2"
              >
      ...
      {navItems.map((item) => (
        <button
          ...
          className={`flex flex-col items-center justify-center w-full h-full ${
            path === item.path ? 'text-[#2AC1BC]' : 'text-gray-400'
          }`}
        >
          {item.icon}
...
const HomePage = ({ user, posts }: { user: UserState, posts: CommunityPost[] }) => {
  ...
  const getDisplayPosts = () => {
      ...
      if (activeTab === 'popular') {
          ...
          filtered = posts.filter(p => p.isNotice);
      } else {
          // 'all': Show Notices first, then others
    ...
    <div className="pb-20 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white px-4 pt-2 pb-0 mb-3 border-b border-gray-100">
         <div className="flex items-center justify-between mb-2">
            <h2 className="font-bold text-lg text-gray-800">맘커뮤니티</h2>
         </div>
         <div className="flex gap-6">
             <button 
                onClick={() => setActiveTab('all')}
                className={`pb-3 text-sm font-bold border-b-2 transition-colors ${activeTab === 'all' ? 'text-gray-900 border-gray-900' : 'text-gray-400 border-transparent'}`}
             >
                 전체
             </button>
             <button 
                onClick={() => setActiveTab('popular')}
                className={`pb-3 text-sm font-bold border-b-2 transition-colors ${activeTab === 'popular' ? 'text-red-500 border-red-500' : 'text-gray-400 border-transparent'}`}
             >
                 인기글
             </button>
             <button 
                onClick={() => setActiveTab('notice')}
                className={`pb-3 text-sm font-bold border-b-2 transition-colors ${activeTab === 'notice' ? 'text-[#2AC1BC] border-[#2AC1BC]' : 'text-gray-400 border-transparent'}`}
             >
                 공지사항
             </button>
         </div>
            ...
            {displayPosts.map(post => {
               ...
               <div 
                 ...
                 className={`p-4 rounded-xl shadow-sm border ${
                    post.isNotice 
                        ? 'bg-teal-50/50 border-teal-100' // Notice Style
               ...
               >
                  ...
                  <div className="flex items-center gap-2 mb-2">
                     {!post.isNotice && (
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${post.isPopular ? 'bg-red-50 text-red-500' : 'bg-gray-100 text-gray-500'}`}>
                            {post.isPopular ? 'HOT' : '일상'}
                        </span>
                     )}
                     <h3 className={`text-sm truncate flex-1 text-gray-800 ${post.isNotice ? 'font-black text-base' : 'font-bold'}`}>
                        {post.title}
                     </h3>
                     <span className="text-xs text-gray-400">{post.timeAgo}</span>
                  </div>
                  
                  {/* For notices, we might show full content or collapsible. For now standard truncating */}
                  <p className={`text-xs text-gray-600 line-clamp-2 mb-3 leading-relaxed ${post.isNotice ? 'text-gray-700 font-medium' : ''}`}>
                      {post.content}
                  </p>
                  
                  {post.imageUrl && (
                      <img src={post.imageUrl} alt="post" className="w-full h-32 object-cover rounded-lg mb-3" />
                  )}
                  ...
                  <div className={`flex items-center justify-between text-xs pt-2 ${post.isNotice ? 'border-t border-teal-100/50' : 'border-t border-gray-50'}`}>
                     <div className="flex items-center gap-1">
                        <span className={`font-bold ${isAdmin ? 'text-gray-900' : 'text-gray-600'}`}>
                            {post.authorName}
                        </span>
                        {/* Admin Badge */}
                        {isAdmin && (
                            <span className="text-[10px] bg-[#2AC1BC] text-white px-1.5 py-0.5 rounded font-bold ml-0.5 flex items-center gap-0.5">
      ...
      <button 
        ...
        className="fixed bottom-20 right-4 bg-[#2AC1BC] text-white p-4 rounded-full shadow-lg hover:bg-[#209692] transition-colors z-40"
      >
        <PenSquare size={24} />
      </button>
    </div>
        ...
        <div className="min-h-screen bg-gray-50 pb-20">
            <div className="bg-white p-5 mb-2">
                ...
                <div className="flex gap-2">
                    {user.role === 'provider' ? (
                       <button onClick={() => navigate('/provider-edit')} className="flex-1 bg-gray-100 py-2 rounded-lg text-xs font-bold text-gray-600">업체 정보 관리</button>
                    ) : (
                       <button className="flex-1 bg-gray-100 py-2 rounded-lg text-xs font-bold text-gray-600">프로필 수정</button>
                    )}
            ...
            {user.role === 'provider' && user.providerInfo && (
                <div className="bg-white p-4 mb-2">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button onClick={() => navigate('/provider-schedule')} className="bg-blue-50 p-4 rounded-xl flex flex-col items-center gap-2">
                            <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-blue-600 shadow-sm"><Settings size={20} /></div>
                            <span className="text-xs font-bold text-gray-700">일정 관리</span>
                        </button>
                        <button onClick={() => navigate('/provider-reviews')} className="bg-purple-50 p-4 rounded-xl flex flex-col items-center gap-2">
                            <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-purple-600 shadow-sm"><Star size={20} /></div>
                            <span className="text-xs font-bold text-gray-700">후기 관리</span>
                        </button>
                        <button onClick={() => navigate('/provider-ads')} className="bg-orange-50 p-4 rounded-xl flex flex-col items-center gap-2">
                            <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-orange-600 shadow-sm"><MegaphoneIcon /></div>
                            <span className="text-xs font-bold text-gray-700">광고 관리</span>
                        </button>
                        <button onClick={() => navigate('/provider-points')} className="bg-green-50 p-4 rounded-xl flex flex-col items-center gap-2">
                            <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-green-600 shadow-sm"><DollarSignIcon /></div>
                            <span className="text-xs font-bold text-gray-700">포인트 충전</span>
                        </button>
                    </div>
                </div>
            )}
...
function App() {
  ...
  const handleLoginSuccess = (userData: any) => {
     // Transform auth user to app user state
